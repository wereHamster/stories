...
It is a block element, semantically ~
...
By default the image is displayed with its intrinsic aspect ratio.

As long as the image is not loaded yet, a placeholder image (based on [SQIP]) is shown in its place.
